...
class Word2Vec:
  ...
  def fit(self, data, key_list = []):
    ...
    self.model = gensim.models.Word2Vec(tokenized_corpus, vector_size=self.vector_length, window=5, min_count=1, workers=5, seed=13518136)
  ...
  def train(self, df, key_list = []):
    ...
    self.max_sequence_length = max([len(i) for i in tokenized_corp], self.max_sequence_length)
    if not self.model:
